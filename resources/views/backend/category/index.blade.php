    ...
    <table id="categoryTable" class="display" style="width:100%">
        ...
        <tbody>
           ...
           <tr>
               ...
               <td>
                    <a href="{{ route('admin.category.edit', $row->id) }}" class="btn btn-outline-primary btn-sm">
                    <i class="fas fa-edit"></i></a>

                    <form action="{{ route('admin.category.destroy', $row->id) }}" id="delete_form" method="POST">
                        @csrf
...
@section('js')
    ...
    <script>
        function update_published(el){
            ...
            $.post('{{ route('admin.category.inactive') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                //console.log(data);
                if(data == 1){
                //    Swal.fire(
                //     'Good job!',
